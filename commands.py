import asyncio, io, os, logging, subprocess, shutil
...
def _run_ytdlp_audio(query: str, out_dir: str) -> tuple:
    ...
    with yt_dlp.YoutubeDL(opts) as ydl:
        ...
        if isinstance(info, dict) and 'entries' in info: info = info['entries'][0]
...
async def setup_handlers(client, phone):
    ...
    async def transcribe_voice(event):
        if not event.is_reply: await event.edit("**• يرجى الرد على رسالة صوتية**"); return
        reply = await event.get_reply_message()
        if not reply.voice and not reply.audio: await event.edit("**• الرد على رسالة صوتية فقط**"); return
    ...
    async def youtube_audio(event):
        ...
        try: info, filepath = await loop.run_in_executor(_DOWNLOAD_EXECUTOR, _run_ytdlp_audio, query, TEMP_DIR)
        except FileNotFoundError as e: await event.edit(f"**• {e}**"); return
        except Exception as e: await event.edit(f"**• فشل التحميل:**\n{str(e)[:200]}"); return
        try:
            dur = format_duration(info.get('duration', 0))
            caption = f"᥉᥆ᥙɾᥴꫀ Ϙƚһ᥆ꪀ\n• {dur} | ᥲᥙძᎥ᥆"
            await client.send_file(event.chat_id, filepath, caption=caption, attributes=[DocumentAttributeAudio(duration=info.get('duration',0), title=info.get('title',''), performer=info.get('uploader',''))])
            await event.delete()
        except Exception as e: await event.edit(f"**• فشل الإرسال:**\n{str(e)[:200]}")
        finally:
    ...
    async def video_download(event):
        ...
        loop = asyncio.get_event_loop()
        try: info, filepath = await loop.run_in_executor(_DOWNLOAD_EXECUTOR, _run_ytdlp_video, query, TEMP_DIR)
        except FileNotFoundError as e: await event.edit(f"**• {e}**"); return
        except Exception as e: await event.edit(f"**• فشل تحميل الفيديو:**\n{str(e)[:200]}"); return
        try:
            dur = format_duration(info.get('duration', 0))
            caption = f"᥉᥆ᥙɾᥴꫀ Ϙƚһ᥆ꪀ\n• {dur} | ᥎Ꭵძꫀ᥆"
            await client.send_file(event.chat_id, filepath, caption=caption, attributes=[DocumentAttributeVideo(duration=info.get('duration',0), w=info.get('width',0), h=info.get('height',0), supports_streaming=True)])
            await event.delete()
        except Exception as e: await event.edit(f"**• فشل الإرسال:**\n{str(e)[:200]}")
        finally:
            try: os.remove(filepath)
    ...
    async def pinterest_download(event):
        ...
        loop = asyncio.get_event_loop()
        try: info, filepath = await loop.run_in_executor(_DOWNLOAD_EXECUTOR, _run_ytdlp_general, url, TEMP_DIR)
        except FileNotFoundError as e: await event.edit(f"**• {e}**"); return
        except Exception as e: await event.edit(f"**• فشل تحميل بنترست:**\n{str(e)[:200]}"); return
        try:
    ...
    async def who_in_call(event):
        if not event.is_group: await event.edit("**• الأمر يعمل في المجموعات فقط**"); return
        await event.edit("**• جاري جلب المشاركين...**")
        try:
            full_chat = await client.get_entity(event.chat_id)
            if not hasattr(full_chat, 'call') or not full_chat.call: await event.edit("**• لا توجد مكالمة نشطة**"); return
            call = full_chat.call
            input_call = InputGroupCall(id=call.id, access_hash=call.access_hash)
            result = await client(GetGroupParticipants(call=input_call, ids=[], sources=[], offset='', limit=100))
            users = []
            for p in result.participants:
                uid = p.user_id if hasattr(p, 'user_id') else p.peer.user_id
                try:
                    ...
                    users.append(name)
                except: users.append(str(uid))
            if not users: await event.edit("**• لا يوجد مشاركين حاليًا**")
            else:
                text = "**👥 الموجودون في المكالمة:**\n"
                for i, name in enumerate(users, 1): text += f"{i}. {name}\n"
                await event.edit(text)
        except Exception as e: await event.edit(f"**• خطأ: {str(e)[:100]}**")
